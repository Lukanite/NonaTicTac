...
public class Board {
	...
	public void draw(Graphics g) {
		...
		for (int y = 0; y < 3; y++) {
			for (int x = 0; x < 3; x++) {
				...
				if (spaces[y*3+x]==1) drawX(g, xpos+(x*size/3)+10*size/500, ypos+(y*size/3)+10*size/500, (int)(size/3.15));
				else if (spaces[y*3+x]==2) drawO(g, xpos+(x*size/3)+10*size/500, ypos+(y*size/3)+10*size/500, (int)(size/3.15));
			}
		}
	}
	public void drawTicTac(Graphics g, int startx, int starty, int size) {
		g.setColor(Color.BLACK);
		if (winner>0 || isFull()) g.setColor(new Color(128,128,128));
	...
	public void drawX(Graphics g, int startx, int starty, int size) {
		...
		if (winner>0 || isFull()) g.setColor(new Color(255,128,128));
	...
	public void drawO(Graphics g, int startx, int starty, int size) {
		...
		if (winner>0 || isFull()) g.setColor(new Color(128,128,255));
	...
	public void checkWin() {
		...
		for (int p = 1; p<3; p++) {
			//check horizontal
			for (int y = 0; y<3; y++) {
				if (spaces[y*3]==p && spaces[y*3+1]==p && spaces[y*3+2]==p) win=p;
			}
			//check vertical
			for (int x = 0; x<3; x++) {
				if (spaces[x]==p && spaces[x+3]==p && spaces[x+6]==p) win = p;
			}
			//check diagonal
			if (spaces[2]==p && spaces[4]==p && spaces[6]==p) win = p;
			if (spaces[0]==p && spaces[4]==p && spaces[8]==p) win = p;
		}
		this.winner = win;
		if (winner>0) {
			active = false;
			...
			if (isFull()) active = false;
		}
	}
	public boolean isFull() {
		boolean full = true;
		for (int space:spaces) if (space==0) full=false;
		return full;
	}
